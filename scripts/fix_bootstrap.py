#!/usr/bin/env python3
"""
fix_bootstrap.py — [已废弃] 历史一次性修复工具

2026-08-06 目录重组 (flash 包移入 flash/ 子目录, 标准包布局) 后,
bootstrap 统一为「向上搜索 pyproject.toml 找项目根, 将项目根加入 sys.path」,
本脚本的目标模式已不存在, 仅保留作历史参考。
"""
...
def process_file(filepath: Path) -> bool:
    ...
    content = content.replace(
        '_SCENARIOS_ROOT = _ROOT / "test" / "scenarios"', '_SCENARIOS_ROOT = _ROOT / "test" / "scenarios"'
    )
