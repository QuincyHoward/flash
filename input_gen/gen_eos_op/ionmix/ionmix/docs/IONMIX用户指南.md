...
> **⚠️ 内容说明（必读）**：本文档由 **AI（人工智能）辅助整理与编写**，内容综合自 IONMIX 原始论文（MacFarlane 1989）、Fortran 源代码（`abjt_03.f`）及项目内技术文档。尽管已尽力核对，AI 生成内容仍可能存在遗漏或偏差。**专业决策（仿真参数设定、工程/科研计算、论文引用等）请务必以原始文献、IONMIX 源代码及官方手册为准，并仔细人工核对后再采用。**

## 快速定位（已熟悉理论？直达源码操作）

> 若您已掌握等离子体 EOS / 不透明度理论，可跳过物理推导，直接跳到以下实操章节：

- **写输入文件** → [§4 输入文件 (IONMXINP)](#4-输入文件-ionmxinp)（NAMELIST 参数 / `isw` 开关 / 完整示例）
- **看代码流程** → [§3 代码结构](#3-代码结构)（主程序流程 + 全部子程序一览）
- **用 Python 生成** → [§6.1 IONMIXInputGen 类](#61-ionmixinputgen-类)（API + 生成流程）
- **编译与运行** → [§7.4 编译说明](#74-编译说明) · [§7.3 手动运行](#73-手动运行) · [§7.2 Python 自动运行](#72-python-自动运行)
...
位置: `Ionmix_Gen/Ionmix_Gen.py`

#### 核心 API

```python
from Ionmix_Gen import IONMIXInputGen
...
| `generate_input(outputfilename)` | **生成输入文件 → 执行 abjt_03 → 重命名输出** |

#### 生成流程

```
IONMIXInputGen.generate_input(outputfilename)
  │
  ├── 生成 ionmxinp 到 ../Ionmix/ 目录
  │
  ├── 清理输出目录（保留 abjt_03, *.cn4 等）
  │
  ├── cd ../Ionmix/
  │
  ├── 执行 ./abjt_03
  │
  ├── 将 eos.cn4 重命名为 outputfilename
  │
  └── 返回生成的输入文件内容
```
...
# 运行
gen.generate_input('my_output.cn4')
```
...
```
ionmix/
├── abjt_03                ← IONMIX 可执行文件
├── abjt_03.f / .f.txt     ← FORTRAN 源代码
...
Ionmix_Gen/
├── Ionmix_Gen.py          ← 核心 Python 生成器
├── Ionmix_Gen_MyWork.py   ← 典型示例调用脚本
├── Ionmix_Gen_ReDo.py     ← 示例生成器类
├── Ionmix_Gen_ReDo_Fast.py← 快速版
├── Ionmix_Gen_paper.py    ← 论文示例
└── debugfiles/            ← 调试文件目录
```
...
```bash
cd Ionmix_Gen/
python Ionmix_Gen_MyWork.py
```

自动流程：
1. 创建 `IONMIXInputGenerator` 或 `IONMIXInputGen` 实例
2. 设置参数（调用 `example_CH()` 等方法）
3. 生成 `ionmxinp` 到 `../Ionmix/`
4. 自动执行 `./abjt_03`（程序会自动进入目录执行）
5. 将 `eos.cn4` 重命名为指定文件名
...
>
> **信息来源**:
> - MacFarlane (1989) 原始论文（`macfarlane1989.pdf`）
> - IONMIX Fortran 源代码（`abjt_03.f`）
> - Python 生成器（`Ionmix_Gen.py` 及变体）
