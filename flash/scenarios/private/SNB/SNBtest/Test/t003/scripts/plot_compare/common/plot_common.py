"""t003 绘图对比 —— 共享层 (数据加载 / 单位换算 / 绘图规范)

本模块是 t003 所有**绘图脚本**的唯一公共依赖, 不产生任何图像文件。
...
**离线复现**, 两腿口径完全一致。理由是 t003 的特殊情形:

  t003 两腿共用同一份 Config (含作者声明的 `NELE` VARIABLE), 因此 chk 里两腿
...
# ── t003 common 层 (参数唯一来源) ─────────────────────────────
_T003 = Path(__file__).resolve().parents[3]
...
# 仓库根 (pyproject.toml 锚点) 已由 t003_common 注入 sys.path
...
# 物种 A / Z (权威来源: t003_common.PARAMS)
# ★ t003 为示例的 4 物种体系: cham(He) 腔体 + tar1/tar2/tar3(CH 靶与示踪层)。
...
def results_dir(sub: str = "") -> Path:
    """绘图输出目录: t003/results/[<sub>/], 不存在则创建。"""
