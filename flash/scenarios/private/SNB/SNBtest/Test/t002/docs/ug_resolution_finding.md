...
|---|---|---|---|---|---|---|---|---|---|---|
| A1 | 固定块 | `-nxb=64` | 64 | 8 | – | 512 | **512** ✅ | 0.9766 µm | 0.9766 µm | 12.47 s |
| A2 | 固定块 | `-nxb=128` | 128 | 8 | – | 1024 | **1024** ✅ | 0.4883 µm | 0.4883 µm | 54.05 s |
| A3 | 固定块 | `-nxb=128` | 128 | 16 | – | 2048 | **2048** ✅ | 0.2441 µm | 0.2441 µm | 164.46 s |
| A4 | 非固定块 | `-nofbs` | – | 8 | 1024 | 1024 | **1024** ✅ | 0.4883 µm | 0.4883 µm | 56.14 s |
| A5 | 非固定块 | `-nofbs` | – | 16 | 1024 | 1024 | **1024** ✅ | 0.4883 µm | 0.4883 µm | 46.56 s |
...
3. **A2 vs A4（同为 1024 格 / 8 核，两种模式）**：格数、dx、解完全一致；
   墙钟 54.05 s vs 56.14 s（差别来自编译期数组维度不同带来的内存/缓存效应）。
   → 两种模式在**同一分辨率**下给出同一物理结果，可互换。

4. **并行效率**：A4（1024 格 / 8 块）56.14 s vs A5（1024 格 / 16 块）46.56 s ——
   核数翻倍只快 **17%**。UG 是"一块一进程"，块数增加会带来守卫单元通信开销，
   小规模算例下**核数收益很低**（与 SNBOneCH 的 WSL 探测结论一致）。

5. **分辨率的价格**：512 → 1024 格，墙钟 12.47 → 54.05 s（**4.3×**）。
