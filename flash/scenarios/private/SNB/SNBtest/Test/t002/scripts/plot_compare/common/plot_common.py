"""t002 绘图对比 —— 共享层 (数据加载 / 单位换算 / 绘图规范)

本模块是 t002 所有**绘图脚本**的唯一公共依赖, 不产生任何图像文件。
...
其中 X_s 是物种质量分数 (chk 里的 `cham` / `targ`)。本模块对该式做**离线复现**,
两腿口径完全一致 —— 尽管 chk 里 SNB 腿**原生带 `nele` 场**, 仍统一走离线推导,
理由:
  1. FL-SH 腿**无** NELE_VAR (标准树 diff_advanceTherm 不写该场), 用原场会造成
     两腿口径不对称;
  2. 实测 SNB 原生 nele 与离线推导有 ~1% 差异 (原生场在特定分支才刷新, 存在
     陈旧值), 离线推导反而更自洽。
...
# ── t002 common 层 (参数唯一来源) ─────────────────────────────
_T002 = Path(__file__).resolve().parents[3]
...
# 仓库根 (pyproject.toml 锚点) 已由 t002_common 注入 sys.path
...
# 物种 A / Z (权威来源: t002_common.PARAMS)
SPECIES_A_Z: Dict[str, Tuple[float, float]] = {
    "cham": (C.PARAMS["cham"]["A"], C.PARAMS["cham"]["Z"]),
    "targ": (C.PARAMS["targ"]["A"], C.PARAMS["targ"]["Z"]),
}
...
def results_dir(sub: str = "") -> Path:
    """绘图输出目录: t002/results/[<sub>/], 不存在则创建。"""
