"""
CHTi2 场景 — 1D 多薄层示踪靶 (纯 CH 基体 (无屏蔽层)) 仿真
═══════════════════════════════════════════════════════════════════
...
VCH_ml_F = V 屏蔽层 + 关辐射。
本脚本: shld=CH, Ti 层=tar2 (Ti-BADGER-TOPS.cn4, rho 4.54 g/cm^3), 辐射: MGD 10 能群开启。
  ...
  * 8 物种标记: cham/shld/samp/tar1/tar2/tar3/tar4/tar6。固体层初始均为
    ...
    物种标记以便诊断追踪。
  * 辐射: MGD 10 能群开启；tabular EOS/opacity (ionmix4)

用法:
  cd <flash 包目录>
  python flash/scenarios/private/tracer/CHTi/CHTi2.py               # 默认 tmax=1.0e-11
  python flash/scenarios/private/tracer/CHTi/CHTi2.py --tmax 1.6e-9 # 正式运行
  # 或 python -m flash.scenarios.private.tracer.CHTi.CHTi2
"""
...
SCENE_NAME = "CHTi2"        # 场景名 → SIM_NAME/par 文件名/图题/SLURM 作业名
