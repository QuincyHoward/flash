...
用法:
  cd <flash 包目录>
  python -m flash.scenarios.private.tracer.layer_tracer_Ti.layer_tracer_Ti
"""
...
def build_species_defs(layer_samp_um: float) -> List[dict]:
    """构建 4 物种定义：cham=He，shld=V，samp=CH，targ=Ti。
    ...
    return [
        ...
        {"name": "shld", "file": v_file, "rho": 6.11, "A": 50.9415, "Z": 23,
         "radius": 1.0e-5},
        # 示踪首层 CH (厚度 = layer_samp_um)
        {"name": "samp", "file": ch_file, "rho": 1.0, "A": 6.509, "Z": 3.5,
         "radius": samp_cm, "height": 5.0e-3},
        # 靶芯层 Ti (0.1um)
        {"name": "targ", "file": ti_file, "rho": 4.54, "A": 47.867, "Z": 22,
...
# ── 区域边界：按物种几何累积生成空间分层 ───────────────────
def build_regions(species_defs: List[dict]) -> List[tuple]:
    """按空间顺序生成分层区域：shld → samp(front) → targ → samp(rear)。

    顺序固定为屏蔽层、示踪首层、靶芯、示踪尾层（对应 initBlock 的
    bound1..bound5 逐段累加）。厚度来自各物种的 radius/height。
    """
    shld = next(s for s in species_defs if s["name"] == "shld")
    samp = next(s for s in species_defs if s["name"] == "samp")
    targ = next(s for s in species_defs if s["name"] == "targ")
    r0 = 0.0
    r1 = r0 + shld["radius"]
    r2 = r1 + samp["radius"]
    r3 = r2 + targ["radius"]
...
def generate_input_files(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    block_gen = BlockGenerator(
        simulation_name="LaserSlab_Ti", sim_path=sim_path,
