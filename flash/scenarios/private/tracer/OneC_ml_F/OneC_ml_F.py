"""
OneC_ml_F 场景 — 1D 多薄层单一纯材料 (碳 C) 示踪靶烧蚀仿真
═══════════════════════════════════════════════════════════════════

由 OneSi_ml 场景派生 (2026-09-02)：结构/物种/几何完全一致, 唯一物理差异是
**全部固体层 (shld/samp/tar1/tar2/tar3/tar4/tar6) 为同一种纯材料 C**
(密度 3.515 g/cm^3 常温固体密度, EOS/不透明度表 Z06_1.00-20260902_2228.cn4,
A=12.011, Z=6) — 单一纯材料烧蚀仿真, shld/tar* 物种标记仅用于
诊断追踪不同深度的材料运动 (无成分界面, 只有物种标记界面)。

与 OneCH_ml 的唯一物理差异: 固体层材质 CH → C (含 shld/samp/tar* 全部
固体物种); 腔室 cham 仍为稀氦 He (1e-6 g/cm^3)。
  ...
  * 8 物种 12 区分层（delta=0.1um, L1=1um, L2=2um, L3=3um, L4=4um,
    L6=6um, D=50um; 示踪层间距均为 samp）:
      ...
      0        < x < delta         shld [碳 C, 3.52 g/cm^3]  ← 表面层标记
      delta    < x < L1            samp [碳 C, 3.52 g/cm^3]
      L1       < x < L1+delta      tar1 [碳 C]  ← 示踪薄层 1
      L1+delta < x < L2            samp [碳 C]
      L2       < x < L2+delta      tar2 [碳 C]  ← 示踪薄层 2
      L2+delta < x < L3            samp [碳 C]
      L3       < x < L3+delta      tar3 [碳 C]  ← 示踪薄层 3
      L3+delta < x < L4            samp [碳 C]
      L4       < x < L4+delta      tar4 [碳 C]  ← 示踪薄层 4
      L4+delta < x < L6            samp [碳 C]
      L6       < x < L6+delta      tar6 [碳 C]  ← 示踪薄层 6
      L6+delta < x < L6+delta+D    samp [碳 C]
      其余 (x<-0.04 域外 / x>56.1um)  cham [氦 He]

  * 8 物种标记: cham/shld/samp/tar1/tar2/tar3/tar4/tar6。固体层初始均为
    常温 (290.11375 K) 密度 3.52 g/cm^3 纯 C; 各标记层物质相同, 用独立
...
用法:
  cd <flash 包目录>
  python -m flash.scenarios.private.tracer.OneC_ml_F.OneC_ml_F   # 默认 tmax=1.0e-11
  python flash/scenarios/private/tracer/OneC_ml_F/OneC_ml_F.py --tmax 1.6e-9
"""
...
SCENE_NAME = "OneC_ml_F" # 场景名 → SIM_NAME/par 文件名/图题/SLURM 作业名
MAT_LABEL = "C"   # 固体材料标签 (碳)
MAT_TABLE = "Z06_1.00-20260902_2228.cn4"   # EOS/不透明度表 (表名 1.00 为生成标称密度)
MAT_RHO = 3.515        # 固体密度 g/cm^3 (常温固体密度; 表内密度网格为离子数密度, 覆盖充足)
MAT_A = 12.011       # 摩尔质量 g/mol
MAT_Z = 6              # 核电荷数
