"""
TiTi1 场景 — 1D 多薄层示踪靶 (Ti 屏蔽层 + 1um 深 Ti 示踪层) 仿真
═══════════════════════════════════════════════════════════════════
...
TiTi 家族 (不同深度单独一个脚本):
  TiTi1  tar1 = Ti @1um   (本脚本)
  TiTi2  tar2 = Ti @2um
  TiTi3  tar3 = Ti @3um
全部由 tracer/build_titi_family.py 从本模板换 "场景差异参数块" 生成,
物理设置除 Ti 示踪层深度外完全一致。
...
用法:
  cd <flash 包目录>
  python flash/scenarios/private/tracer/TiTi/TiTi1.py               # 默认 tmax=1.0e-11
  python flash/scenarios/private/tracer/TiTi/TiTi1.py --tmax 1.6e-9 # 正式运行
  # 或 python -m flash.scenarios.private.tracer.TiTi.TiTi1
"""
...
SCENE_NAME = "TiTi1"        # 场景名 → SIM_NAME/par 文件名/图题/SLURM 作业名
