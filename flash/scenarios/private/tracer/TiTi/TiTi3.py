"""
TiTi3 场景 — 1D 多薄层示踪靶 (Ti 屏蔽层 + 3.0um 深 Ti 示踪层) 仿真
═══════════════════════════════════════════════════════════════════
...
TiTi 家族 (不同深度单独一个脚本):
  ...
  TiTi2  tar2 = Ti @2um
  TiTi3  tar3 = Ti @3um
全部由 tracer/build_titi_family.py 从 TiTi1 模板换块生成, 本脚本: Ti 层=tar3 (Ti-BADGER-TOPS.cn4, rho 4.54 g/cm^3)。
...
用法:
  cd <flash 包目录>
  python flash/scenarios/private/tracer/TiTi/TiTi3.py               # 默认 tmax=1.0e-11
  python flash/scenarios/private/tracer/TiTi/TiTi3.py --tmax 1.6e-9 # 正式运行
  # 或 python -m flash.scenarios.private.tracer.TiTi.TiTi3
"""
...
# ── BEGIN SCENE VARS (TiTi 家族各脚本仅此块不同; build_titi_family.py 换块生成) ──
SCENE_NAME = "TiTi3"        # 场景名 → SIM_NAME/par 文件名/图题/SLURM 作业名
SHLD_MATERIAL = "Ti"        # "Ti"=钛屏蔽层 (TiTi 系); "CH"=无屏蔽层 (OneCH 系); "V"=钒屏蔽层 (VCH 系)
TI_LAYER = "tar3"           # Ti 示踪层所在物种; None=无 Ti 层 (tar* 全 CH)
...
def build_species_defs(delta_cm: float, L1: float, L2: float, L3: float,
    ...
    或 Ti 4.54 ("Ti", TiTi 系, 与 Ti 示踪层同材质); samp=CH; tar* = CH 标记,
    其中 TI_LAYER 指定的一层为 Ti 示踪层 (Ti-BADGER-TOPS.cn4,
    rho 4.54 g/cm^3 — 同参考 ReDo042sp sim_rhoTarg, A=47.867, Z=22)。
    ...
    **只改 .par 中的参数值即可改变场景几何, 无需改代码**:
    ...
    # 屏蔽层材质 (VCH 系: 钒 V; TiTi 系: 钛 Ti; 纯 CH 系: CH)
    if SHLD_MATERIAL == "V":
        shld_def = {"name": "shld", "file": v_file, "rho": 6.11,
                    "A": 50.9415, "Z": 23, "radius": delta_cm}
    elif SHLD_MATERIAL == "Ti":
        shld_def = {"name": "shld", "file": ti_file, "rho": 4.54,
                    "A": 47.867, "Z": 22, "radius": delta_cm}
    else:
...
def generate_input_files(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    # 覆写: 屏蔽层材质绑定 (VCH 系: shld=V 6.11 + V 表; TiTi 系: shld=Ti 4.54
    # + Ti 表, 与 Ti 示踪层同绑定; 纯 CH 系: 模板默认 sim_rhoShld=1 + CH 表
    # 已正确, 无需覆写)
    if SHLD_MATERIAL == "V":
        ...
        par_gen.set("sim_rhoShld", 4.54)   # Ti 真实密度 (同 Ti 示踪层, 模板占位 1 为 CH)
        par_gen.set("eos_shldTableFile", "Ti-BADGER-TOPS.cn4")
        par_gen.set("op_shldFileName", "Ti-BADGER-TOPS.cn4")
    ...
    # Ti 表条件覆盖两个引用来源: TI_LAYER 示踪层 + SHLD_MATERIAL="Ti" 屏蔽层
    # (tables 复制清单必须覆盖所有 par 引用分支, 漏表即 DRIVER_ABORT)
