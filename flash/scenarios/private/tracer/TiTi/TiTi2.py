"""
TiTi2 场景 — 1D 多薄层示踪靶 (Ti 屏蔽层 + 2.0um 深 Ti 示踪层) 仿真
═══════════════════════════════════════════════════════════════════
...
TiTi 家族 (不同深度单独一个脚本):
  ...
  TiTi2  tar2 = Ti @2um
  TiTi3  tar3 = Ti @3um
全部由 tracer/build_titi_family.py 从 TiTi1 模板换块生成, 本脚本: Ti 层=tar2 (Ti-BADGER-TOPS.cn4, rho 4.54 g/cm^3)。
  ...
  * 8 物种 12 区分层（delta=0.1um, L1=1um, L2=2um, L3=3um, L4=4um,
    L6=6um, D=50um; 示踪层间距均为 samp）:
      ...
      L2       < x < L2+delta      tar2 [钛 Ti, 4.54]  ← Ti 示踪层 @2.0um
...
用法:
  cd <flash 包目录>
  python flash/scenarios/private/tracer/TiTi/TiTi2.py               # 默认 tmax=1.0e-11
  python flash/scenarios/private/tracer/TiTi/TiTi2.py --tmax 1.6e-9 # 正式运行
  # 或 python -m flash.scenarios.private.tracer.TiTi.TiTi2
"""
...
SCENE_NAME = "TiTi2"        # 场景名 → SIM_NAME/par 文件名/图题/SLURM 作业名
SHLD_MATERIAL = "Ti"        # "Ti"=钛屏蔽层 (TiTi 系); "CH"=无屏蔽层 (OneCH 系); "V"=钒屏蔽层 (VCH 系)
TI_LAYER = "tar2"           # Ti 示踪层所在物种; None=无 Ti 层 (tar* 全 CH)
