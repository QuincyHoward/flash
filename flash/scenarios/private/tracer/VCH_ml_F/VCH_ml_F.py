"""
VCH_ml_F 场景 — 1D 多薄层示踪靶 (CH 基体 + V 屏蔽层) 仿真
═══════════════════════════════════════════════════════════════════
...
VCH_ml_F = V 屏蔽层 + 关辐射。
本脚本: shld=V, Ti 层=无 (tar* 全 CH), 辐射: 关闭 (F 变体: rt_useMGD/useOpacity/useRadTrans 均 .false.)。
  ...
  * 8 物种 12 区分层（delta=0.1um, L1=1um, L2=2um, L3=3um, L4=4um,
    L6=6um, D=50um; 示踪层间距均为 samp）:
      ...
      0        < x < delta         shld [钒 V, 6.11]  (V 屏蔽层, 同 VCH_ml)
...
用法:
  cd <flash 包目录>
  python flash/scenarios/private/tracer/VCH_ml_F/VCH_ml_F.py               # 默认 tmax=1.0e-11
  python flash/scenarios/private/tracer/VCH_ml_F/VCH_ml_F.py --tmax 1.6e-9 # 正式运行
  # 或 python -m flash.scenarios.private.tracer.VCH_ml_F.VCH_ml_F
"""
...
SCENE_NAME = "VCH_ml_F"     # 场景名 → SIM_NAME/par 文件名/图题/SLURM 作业名
SHLD_MATERIAL = "V"        # "CH"=无屏蔽层 (纯 CH, OneCH 系); "V"=钒屏蔽层 (VCH 系)
TI_LAYER = None         # Ti 示踪层所在物种; None=无 Ti 层 (tar* 全 CH)
