...
# 时空插值引擎 (flash.scenarios.interpolator, 随包发布):
# h5py 属于 [full] extra, 未安装时降级, 不阻塞场景注册。
try:
...
except ImportError:  # pragma: no cover - 最小环境 (无 h5py)
    def _build_variable_grid(*_a, **_k):
        raise RuntimeError(
            "ch_center 网格/插值功能需要 h5py 与 numpy, 请安装: pip install 'flash-sim[full]'"
        )
