!!****if* source/Simulation/SimulationMain/QC/LaserSlab_custom/Simulation_initBlock
!!
!! NAME
!!
...
!!  Center's source/Simulation/SimulationMain/QC/LaserSlab_custom/
!!  Simulation_initBlock, adapted for a new physics setup. Per FLASH
!!  License Agreement §4(a), this notice declares that this file has
!!  been changed from the original FLASH Code; the original FLASH
!!  header is preserved intact per §4(c).
  ...
  do k = blkLimits(LOW,KAXIS),blkLimits(HIGH,KAXIS)
     do j = blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
        do i = blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           ...
           if (xcent(i) >= -3.000000e-03 .and. xcent(i) <= 3.000000e-03) then
              species = TARG_SPEC
           end if
