...
def extract_with_yt_2d(hdf5_file, var_name='dens'):
    """使用 yt 提取 2D 数据"""
    if not HAS_YT:
    ...
    # 提取坐标和变量（FLASH 2D 使用柱坐标）
    r_coords = ad[('flash', 'r')].to_value()
    ...
    # 排序并去重（保留第一个出现的）
    df = df.sort_values(['r', 'z']).drop_duplicates(subset=['r', 'z'], keep='first')

    return df['r'].values, df['z'].values, df[var_name].values


def extract_with_flash_hdf5_2d(hdf5_file, var_name='dens'):
    """使用 flash_hdf5.py 提取 2D 数据（yt 风格）"""
    with FlashHDF5File(hdf5_file) as f:
        ...
        if f.ndim == 2:
            ...
            raise ValueError(f"期望 2D 文件，但得到 {f.ndim}D")


def save_yt_baseline_2d(hdf5_file, output_dir):
    """使用 yt 提取数据并保存为基准"""
    if not HAS_YT:
    ...
    x_yt, y_yt, dens_yt = extract_with_yt_2d(hdf5_file, 'dens')
...
def compare_point_by_point_2d(hdf5_file, yt_baseline_file=None):
    ...
    x_hdf5, y_hdf5, dens_hdf5 = extract_with_flash_hdf5_2d(hdf5_file, 'dens')
    ...
    # 4. 排序（按 x, y 坐标）
    print(f"\n4️⃣ 排序数据（按 x, y 坐标）...")
    sort_idx_hdf5 = np.lexsort((y_hdf5, x_hdf5))
    sort_idx_yt = np.lexsort((y_yt, x_yt))
    ...
    coord_match = np.allclose(x_hdf5_sorted, x_yt_sorted, rtol=1e-8, atol=1e-10) and \
                  np.allclose(y_hdf5_sorted, y_yt_sorted, rtol=1e-8, atol=1e-10)
    ...
    # 判断坐标一致性（分档）
    max_coord_diff = max(max_coord_diff_x, max_coord_diff_y)
    if coord_match:
        coord_status = "✅ 完全一致"
    ...
    if not coord_match:
        ...
        if len(mismatch_indices) > 0:
            ...
            for i in mismatch_indices[:10]:
                print(f"     索引 {i}: flash_hdf5=({x_hdf5_sorted[i]:.10e}, {y_hdf5_sorted[i]:.10e}), "
                      f"yt=({x_yt_sorted[i]:.10e}, {y_yt_sorted[i]:.10e}), "
                      f"差异=({coord_diff_x[i]:.6e}, {coord_diff_y[i]:.6e})")
    ...
    if not dens_match:
        ...
        if len(mismatch_indices) > 0:
            print(f"   前 10 个不一致的点:")
            for i in mismatch_indices[:10]:
                print(f"     索引 {i}: x={x_hdf5_sorted[i]:.10e}, y={y_hdf5_sorted[i]:.10e}, "
...
def main():
    """主函数"""
    print(f"{'#'*60}")
    print(f"# 测试 flash_hdf5.py 的 2D 数据提取功能")
    print(f"# 逐点对比 flash_hdf5.py 与 yt 的处理结果")
    print(f"{'#'*60}")

    # 获取输入文件目录
    input_dir = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'inputfiles', 'hdf5files_2d')
    ...
    if len(sys.argv) > 1:
        ...
        hdf5_files = sorted([
            ...
            if f.startswith('lasslab_hdf5_plt_cnt_') and os.path.isfile(os.path.join(input_dir, f))
        ])
        if len(hdf5_files) == 0:
            print(f"\n❌ 错误: 在 {input_dir} 中未找到任何 plt_cnt 文件!")
            return
    ...
    if HAS_YT:
        yt_baseline_file = save_yt_baseline_2d(input_file, output_dir)
    else:
        # 尝试读取已有的基准数据
    ...
    # 逐点对比
    result = compare_point_by_point_2d(input_file, yt_baseline_file)
...
def test_2d_extraction():
    """pytest 测试：验证 2D 数据提取与 yt 一致（需要 HDF5 输入文件）"""
    import pytest
    import os
    ...
    # 运行对比测试
    result = compare_point_by_point_2d(input_file, yt_baseline_file=None)
